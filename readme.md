...
`P(D_{k+1} = 1 | D_k, Z) = exp(α + β_1 D_k + β_2 Z) / (1 + exp(α + β_1 D_k + β_2 Z))`
...
Considering this model, we want to deconfound the `D_{k+1} ~ D_k` association from all family-level confounders (i.e., we seek the estimand `P(D_{k+1} | do(D_k))`). Such confounders could produce networks that are non-causal but instead reflect shared family factors (e.g., genetics, household socioeconomic conditions, parental healthcare-seeking patterns).
...
`P(D_{k+1} = 1 | D_k, Z, \bar{D}_k) = exp(α + β_1 D_k + β_2 Z + β_3 \bar{D}_k) / (1 + exp(α + β_1 D_k + β_2 Z + β_3 \bar{D}_k))`
...
`P(D_{k+1} = 1 | D_k, Z, \bar{D}_k, \bar{Z}) = exp(α + β_1 D_k + β_2 Z + β_3 \bar{D}_k + β_4 \bar{Z}) / (1 + exp(α + β_1 D_k + β_2 Z + β_3 \bar{D}_k + β_4 \bar{Z}))`
