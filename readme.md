...
Considering this model, we want to deconfound the $D_{k+1} \sim D_k$ association from all family-level confounders (i.e., we seek the estimand $P(D_{k+1} \mid do(D_k))$). Such confounders could produce networks that are non-causal but instead reflect shared family factors (e.g., genetics, household socioeconomic conditions, parental healthcare-seeking patterns).  
